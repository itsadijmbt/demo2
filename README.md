...
| **MACAW** | `launch-server.py` | the **ported SecureMCP** server (Snowflake mocked, no creds) the
...
4. **Snowflake STORE policy loaded** in the Console
5. **export MANAGER_USER="buszadi1@gmail.com"**
6. **export MANAGER_PW="test@123"**
7. **From the Demo2/pip install -e .**
8. **In Each Terminal where you have macaw realated dependency or all run : source venv/bin/activate && export MACAW_HOME="/path/to/macaw-client-0.9.9.1-Linux-x86_64-py3.12"**
---
...
```bash
# Terminal 1  the SecureMCP server 
